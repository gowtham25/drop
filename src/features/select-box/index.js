		...
		return <div className="select-box--box">
					<div className="select-box--container">
						<div className="select-box--selected-item">
							{ this.state.selectedList.length === 0 ? "Multi Select" : this.state.selectedList.length > 1 ? this.state.selectedList.length+" out of "+this.state.items.length + " Selected" :  this.state.selectedList[0] }
						</div>
					...
					<div className="select-box--items" style={{display: this.state.showItems ? 'block' : 'none'}}>
						<div className="and-or">
							<input type="radio" id="or" onChange={() => this.changeRadio("or")} value="or" name="operator"/>
							<label htmlFor="or">OR</label>
							<input type="radio" id="and" onChange={() => this.changeRadio("and")} value="and" name="operator"/>
							<label htmlFor="and">AND</label></div>
						{
						this.state.items.map(item => <div key={item.id}> <input type="checkbox" id={"id"+item.id}
